...
use Modules\Article\Http\Repositories\ReserveTemplateRepository;
...
{
    public function __construct(public ReserveTemplateRepository $reserveTemplateRepository, public ArticleRepository $articleRepository)
    {
    }
    ...
    public function betweenDate(ReserveTemplateBetweenDateRequest $request): array
    {
        try {
            ...
            $ArticleRepository = resolve('ArticleRepository');
            /** @var Article $article */
            $article = $ArticleRepository->find($article_id);

            $reserveTemplates = DB::table('reserve_templates')
                ->where('reserve_templates.article_id', $article_id)
                ...
                ->select(
                    ...
                    'reserves.article_id as reserve_article_id',
                    'reserves.user_id',
            ...
            foreach ($reserveTemplates as $template) {
                $data = [
                    ...
                    'price' => $template->price ?? $article->price,
                    ...
                    'reserve' => $template->reserve_id ? [
                        ...
                        'article_id' => $template->reserve_article_id,
            ...
            # select article
            /** @var Article $article */
            $article = Article::query()->with(['urlImages','sports','attributes'])->find($article_id);
            # select article

            return ['article'=>$article,'reserve_templates'=>$reserve_templates];
