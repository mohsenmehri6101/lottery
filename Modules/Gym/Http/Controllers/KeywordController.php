...
class KeywordController extends Controller
{
    ...
    /**
     * @OA\Post (
     *     path="/api/v1/keywords/sync-keyword-to-gym",
     *     tags={"keywords"},
     *     summary="sync keyword to gym",
     *     security={{"bearerAuth":{}}},
     ...
     *  )
     */
    public function syncKeywordToGym(SyncKeywordToGymRequest $request)
    {
        $status = $this->keywordService->syncKeywordToGym($request);
        return $status ? ResponseHelper::responseSuccess() : ResponseHelper::responseFailed();
    }

    /**
     * @OA\Post (
     *     path="/api/v1/keywords/delete-keyword-to-gym",
     *     tags={"keywords"},
     *     summary="delete keyword to gym",
     *     security={{"bearerAuth":{}}},
     *     @OA\Parameter(name="gym_id",in="query",required=true, @OA\Schema(type="integer"),description="gym_id"),
     *     @OA\Parameter(name="detach",in="query",required=false, @OA\Schema(type="integer"),description="detach"),
     *     @OA\Parameter(name="keyword_id",in="query",required=false, @OA\Schema(type="string"),description="keyword_id"),
    ...
    {
        $status = $this->keywordService->deleteKeywordToGym($request);
        return $status ? ResponseHelper::responseSuccess() : ResponseHelper::responseFailed();
    }
